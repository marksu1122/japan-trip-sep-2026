...
path = r'c:\Users\mark6\.gemini\antigravity\scratch\japan-trip-sep-2026\index.html'

with open(path, 'r', encoding='utf-8') as f:
    content = f.read()

# Locate the block for Day 7 (2026-10-02)
# We search from the opening bracket of 2026-10-02 to the opening bracket of 2026-10-03
start_marker = 'date: "2026-10-02",'
end_marker = 'date: "2026-10-03",'

start_idx = content.find(start_marker)
if start_idx == -1:
    print("Error: 2026-10-02 not found")
    exit(1)

# Backtrack to the opening '{' of the day object
block_start = content.rfind('{', 0, start_idx)
if block_start == -1:
    print("Error: Opening bracket for Day 7 not found")
    exit(1)

end_idx = content.find(end_marker, start_idx)
if end_idx == -1:
    print("Error: 2026-10-03 not found")
    exit(1)

# Backtrack to the opening '{' of the Day 8 block, and then find the closing '},' of the Day 7 block
block_end = content.rfind('},', start_idx, end_idx)
if block_end == -1:
    print("Error: Closing bracket for Day 7 not found")
    exit(1)
block_end += 2  # Include '},'
            ...
            events: [
                {
                    ...
                    desc: "早上全家睡到自然醒，散步前往水上公園大樓的 stock。這時段麵包剛出爐、種類最齊全。在極具設計感的冷氣店內享用優雅的早茶，拍照好看，長輩精神也剛好喚醒。",
